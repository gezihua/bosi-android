...
public class LeaMsgFragment extends NotifyCenterFragment {
    ...
    MSGHISTORY mHistory = new MSGHISTORY();
    ...
    protected void afterViewInject() {
        ...
        mHistory.updateDataFromDb(getResources().getString(R.string.update_leavemsg_readed));
    ...
    protected void selectFromLocalDb() {
        ...
        String sqlSendByOther = mActivity.getStringFormat(
                getString(R.string.select_leamsgsendbyotherlist),
                myEmgsAccount);
        // 查询我发出的信息 根据发送给的人分组的集合
        List<ChatMsgBean> mlocalSendByOther = mDbMsgHistory
                .selectDataFromDb(sqlSendByOther);

        if (mlocalSendByOther != null && mlocalSendByOther.size() > 0) {

            this.mList = mlocalSendByOther;
    ...
    protected void onItemClickCallBack(int position) {
        ...
        if(!FriendSystemUtils.isMyFriend(mChatBean.msgFrom, mActivity)){
